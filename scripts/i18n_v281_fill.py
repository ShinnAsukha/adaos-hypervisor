...
ROWS = [
 ...
 # v2.8.1 — Time-machine / ChatOps
 ('Anlık Görüntü Al', 'Take Snapshot', 'Tomar instantánea', 'Snapshot erstellen', '创建快照', 'Prendre un instantané'),
 ('Anlık Görüntüler', 'Snapshots', 'Instantáneas', 'Snapshots', '快照', 'Instantanés'),
 ("ChatOps — Telegram'dan Yönet", 'ChatOps — Manage from Telegram', 'ChatOps — Gestiona desde Telegram', 'ChatOps — Über Telegram verwalten', 'ChatOps — 从 Telegram 管理', 'ChatOps — Gérer depuis Telegram'),
 ('Datacenter\'ı Telegram\'dan AI ile yönet — "vm-3 neden yavaş?", "yeni ubuntu VM aç", "cluster durumu". Rakipler sadece tek-yön uyarı yollar; OXware iki-yön + AI. Telegram bot token\'ı Bildirimler\'den alınır.', 'Manage the datacenter from Telegram with AI — "why is vm-3 slow?", "create a new ubuntu VM", "cluster status". Competitors only push one-way alerts; OXware is two-way + AI. The Telegram bot token comes from Notifications.', 'Gestiona el datacenter desde Telegram con IA — "¿por qué vm-3 va lenta?", "crea una VM ubuntu", "estado del clúster". Los competidores solo envían alertas de una vía; OXware es bidireccional + IA. El token del bot de Telegram viene de Notificaciones.', 'Verwalte das Rechenzentrum von Telegram aus mit KI — „warum ist vm-3 langsam?", „neue Ubuntu-VM erstellen", „Cluster-Status". Konkurrenten senden nur Einweg-Warnungen; OXware ist bidirektional + KI. Der Telegram-Bot-Token stammt aus den Benachrichtigungen.', '通过 AI 从 Telegram 管理数据中心——"vm-3 为什么慢？""新建一台 ubuntu 虚拟机""集群状态"。竞品只单向推送告警；OXware 是双向 + AI。Telegram 机器人令牌来自通知设置。', 'Gérez le datacenter depuis Telegram avec l\'IA — « pourquoi vm-3 est lente ? », « créer une VM ubuntu », « état du cluster ». Les concurrents n\'envoient que des alertes unidirectionnelles ; OXware est bidirectionnel + IA. Le token du bot Telegram provient des Notifications.'),
 ('Değişiklik işlemlerine izin ver (VM oluştur/durdur/sil) — dikkatli', 'Allow mutating operations (create/stop/delete VM) — careful', 'Permitir operaciones de cambio (crear/detener/eliminar VM) — cuidado', 'Ändernde Operationen erlauben (VM erstellen/stoppen/löschen) — Vorsicht', '允许变更操作（创建/停止/删除虚拟机）——谨慎', 'Autoriser les opérations de modification (créer/arrêter/supprimer une VM) — prudence'),
 ('Farkı Göster', 'Show Diff', 'Mostrar diferencia', 'Unterschied anzeigen', '显示差异', 'Afficher la différence'),
 ('Kapalıyken AI yalnızca okuma/sorgu yapar (operatör yetkisi). İzinli chat ID girilmezse herkes (token\'ı bilen) erişebilir — mutlaka ID girin.', 'When off, the AI only reads/queries (operator authority). If no allowed chat IDs are set, anyone who knows the token can access — always set IDs.', 'Cuando está desactivado, la IA solo lee/consulta (autoridad de operador). Si no se establecen IDs de chat permitidos, cualquiera que conozca el token puede acceder — establece siempre IDs.', 'Wenn aus, liest/abfragt die KI nur (Operator-Berechtigung). Ohne erlaubte Chat-IDs kann jeder mit dem Token zugreifen — immer IDs setzen.', '关闭时，AI 仅读取/查询（操作员权限）。若不设置允许的 chat ID，任何知道令牌的人都可访问——务必设置 ID。', 'Désactivé, l\'IA ne fait que lire/interroger (autorité opérateur). Sans IDs de chat autorisés, quiconque connaît le token peut accéder — définissez toujours des IDs.'),
 ('Karşılaştır', 'Compare', 'Comparar', 'Vergleichen', '比较', 'Comparer'),
 ('Tüm cluster yapılandırmasının (VM/ağ/DVS/vApp) zaman damgalı anlık görüntüsü + iki nokta arası git-tarzı fark. Ne, ne zaman değişti gör. Geri yükleme bilgilendiricidir (farkı gösterir).', 'A timestamped snapshot of the whole cluster config (VM/network/DVS/vApp) + a git-style diff between any two points. See what changed and when. Restore is advisory (shows the delta).', 'Una instantánea con marca de tiempo de toda la configuración del clúster (VM/red/DVS/vApp) + una diferencia estilo git entre dos puntos. Ve qué cambió y cuándo. La restauración es informativa (muestra el delta).', 'Ein zeitgestempelter Snapshot der gesamten Cluster-Konfiguration (VM/Netzwerk/DVS/vApp) + ein Git-artiger Diff zwischen zwei Punkten. Sieh, was sich wann geändert hat. Wiederherstellung ist beratend (zeigt das Delta).', '整个集群配置（VM/网络/DVS/vApp）的带时间戳快照 + 任意两点间的 git 式差异。查看何时发生了什么变化。恢复为建议性（显示增量）。', 'Un instantané horodaté de toute la configuration du cluster (VM/réseau/DVS/vApp) + une différence façon git entre deux points. Voyez ce qui a changé et quand. La restauration est indicative (affiche le delta).'),
 ("İzinli Chat ID'ler (virgül)", 'Allowed Chat IDs (comma)', 'IDs de chat permitidos (coma)', 'Erlaubte Chat-IDs (Komma)', '允许的 Chat ID（逗号）', 'IDs de chat autorisés (virgule)'),
 ('Anlık görüntü alındı', 'Snapshot taken', 'Instantánea tomada', 'Snapshot erstellt', '快照已创建', 'Instantané pris'),
 ('Anlık görüntü etiketi (opsiyonel):', 'Snapshot label (optional):', 'Etiqueta de instantánea (opcional):', 'Snapshot-Bezeichnung (optional):', '快照标签（可选）：', 'Étiquette de l\'instantané (optionnel) :'),
 ('İki anlık görüntü seç', 'Select two snapshots', 'Selecciona dos instantáneas', 'Zwei Snapshots wählen', '选择两个快照', 'Sélectionnez deux instantanés'),
 # v2.8.1 — gizli hypervisor tespiti
 ('Baseline Kaydet', 'Save Baseline', 'Guardar línea base', 'Baseline speichern', '保存基线', 'Enregistrer la référence'),
 ('Taramak için "Tara" tıkla.', 'Click "Scan" to scan.', 'Haz clic en "Escanear" para escanear.', 'Zum Scannen auf „Scannen" klicken.', '点击"扫描"开始扫描。', 'Cliquez sur « Analyser » pour lancer l\'analyse.'),
 ('İşletim sisteminin ALTINDA çalışan gizli bir hypervisor (blue-pill / nested rootkit) belirtilerini tarar. Bağımsız vektörler: CPUID hypervisor biti, detect-virt, Secure Boot,', 'Scans for signs of a hidden hypervisor running BENEATH the OS (blue-pill / nested rootkit). Independent vectors: CPUID hypervisor bit, detect-virt, Secure Boot,', 'Busca indicios de un hypervisor oculto ejecutándose POR DEBAJO del SO (blue-pill / rootkit anidado). Vectores independientes: bit hypervisor de CPUID, detect-virt, Secure Boot,', 'Sucht nach Anzeichen eines versteckten Hypervisors UNTER dem Betriebssystem (Blue-Pill / verschachteltes Rootkit). Unabhängige Vektoren: CPUID-Hypervisor-Bit, detect-virt, Secure Boot,', '扫描运行在操作系统之下的隐藏 hypervisor（blue-pill / 嵌套 rootkit）迹象。独立向量：CPUID hypervisor 位、detect-virt、Secure Boot、', 'Recherche des signes d\'un hyperviseur caché s\'exécutant SOUS le système d\'exploitation (blue-pill / rootkit imbriqué). Vecteurs indépendants : bit hyperviseur CPUID, detect-virt, Secure Boot,'),
 ('(en güçlü), kaba zamanlama. Önce "Baseline Kaydet" ile güvenilir referans al; sonraki taramalar bundan sapmayı yakalar.', '(strongest), coarse timing. First capture a trusted reference with "Save Baseline"; later scans catch drift from it.', '(el más fuerte), tiempo aproximado. Primero captura una referencia fiable con "Guardar línea base"; los análisis posteriores detectan la desviación.', '(am stärksten), grobe Zeitmessung. Erfasse zuerst mit „Baseline speichern" eine vertrauenswürdige Referenz; spätere Scans erkennen Abweichungen.', '（最强）、粗略计时。先用"保存基线"获取可信参考；后续扫描据此检测偏移。', '(le plus fort), timing approximatif. Capturez d\'abord une référence fiable avec « Enregistrer la référence » ; les analyses ultérieures détectent la dérive.'),
